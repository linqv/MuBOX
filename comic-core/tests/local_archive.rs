use comic_core::archive::{ArchiveFormat, open_local_archive};
use std::fs::File;
...
#[test]
fn solid_seven_z_archive_is_decoded_once_and_served_from_page_cache() {
    let archive = make_solid_7z(&[
        ("10.jpg", b"ten".as_slice()),
        ("notes.txt", b"skip".as_slice()),
        ("nested/2.png", b"two".as_slice()),
        ("1.jpg", b"one".as_slice()),
    ...
    assert_eq!(b"ten".to_vec(), session.extract_page(2).unwrap());

    // The first solid-page request streams the whole block into the session cache. Destroying the
    // source proves later random reads do not start another prefix decompression.
    File::options()
        .write(true)
        .open(archive.path())
        .unwrap()
