...
为漫画阅读器增加一个当前阅读会话内的横屏切换按钮。用户打开一本漫画后，可以在阅读器控制层里临时进入横屏或退出横屏并恢复全局方向规则；这个选择不写入设置，不影响下一次打开漫画的默认行为。
...
在 `ComicDavApp` 增加 `readerLandscapeModeEnabled`，使用 `rememberSaveable` 保存。`ReaderRoute` 接收该状态和切换回调，并传给 `ReaderScreen`。`ReaderScreen` 只展示按钮并发出回调，不直接修改 Activity 方向。

`ComicDavApp` 根据两个输入统一计算主 Activity 的请求方向：

- 阅读器打开且 `readerLandscapeModeEnabled == true`：请求 `ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE`。
...
- 已处于临时横屏：显示 `退出横屏`，点击后退出临时横屏并恢复全局方向规则。

按钮与现有 `日志`、`关闭`按钮同级。控制层隐藏时按钮也隐藏，保持阅读画面干净。

关闭阅读器、取消打开、按返回退出阅读器时，清空 `readerLandscapeModeEnabled`。这样临时横屏不会泄漏到应用其他页面或下一次阅读。
...
ReaderTopBar button
  ...
  -> ReaderRoute
  -> ComicDavApp.readerLandscapeModeEnabled
...
- 方向映射测试：阅读器横屏开启时返回 `SCREEN_ORIENTATION_LANDSCAPE`；关闭或阅读器未打开时返回全局 `mainAppRequestedOrientation()` 结果。
- Reader UI 控制测试：阅读器顶部控制项包含 `横屏` 或 `退出横屏`，并保留 `日志`、`关闭`。
- 导航状态测试：关闭阅读器的纯函数或可测 helper 会清掉临时横屏状态。
